...
class Encoder(torch.nn.Module):
    def __init__(self, model_name, version='B', layer=None, device='cuda', img_size=224):
        super().__init__()
        self.model_name = model_name
        self.model_config = get_model_config(model_name, version)
        self.model = get_model(model_name, version, device, img_size)
        self.num_layers = self.model_config['layers'] if layer is None else layer
    ...
    def forward_mae(self, x):
        x, mask, ids_restore = self.model.forward_encoder(x, mask_ratio=0, layer=self.num_layers)
        
        x_ = x[:, 1:, :]  # no cls token
        x_ = torch.gather(x_, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, x.shape[2]))  # unshuffle
        img_enc = torch.cat([x[:, :1, :], x_], dim=1)  # append cls token
    ...
    def forward_ibot(self, x):
        ...
        if self.model.fc_norm is not None:
            x[:, 0] = self.model.fc_norm(x[:, 1:, :].mean(1))
        
        return_all_tokens = self.model.return_all_tokens

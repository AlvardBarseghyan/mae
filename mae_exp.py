...
model_mae = Encoder('mae', img_size=img_size, device=device).eval()
...
for ann in tqdm(annot['images'], total=len(annot['images'])):
    ...
    file = ann['file_name'].replace(
        '/leftImg8bit/', '/vision/images/cs/full_ds/')
    # print(file)
    img = cv2.imread(file)
    ...
    with torch.no_grad():
        img_enc = model_mae(img.to(device))
    ...
    embeds.append(img_enc.squeeze(0).detach().cpu().numpy())
    black = cv2.resize(ann['black_image'], (128, 64),
...
np.save('./embeddings/cs_full/mae_val_embeds.npy', embeds)
np.save('./embeddings/cs_full/mae_val_labels.npy', labels)
